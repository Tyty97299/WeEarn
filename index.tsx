...
import { Coins, Wallet, Lock, X, CheckCircle, AlertCircle, Zap, TrendingUp, TrendingDown, Minus, Clock, MousePointerClick, Bell } from "lucide-react";
...
const App = () => {
  ...
  useEffect(() => {
    const updateMarket = () => {
        ...
        // Logic Notification (si activé, si marché haussier, et si pas déjà notifié pour ce bloc)
        const isBullish = rand >= 0.70; // 0.70+ couvre Bull Run et Moon
        
        // On vérifie notificationsEnabled via une ref ou directement (ici directement car useEffect dépend de notificationsEnabled ci-dessous si on l'ajoute, 
        // ou on utilise la valeur courante dans la closure si updateMarket est recréé)
        // Note: Pour simplifier, on lit la valeur courante.
  ...
  }, [notificationsEnabled]); // Dépendance ajoutée pour que la closure updateMarket voit la bonne valeur de notificationsEnabled
